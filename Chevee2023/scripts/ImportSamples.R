importfiles <- function(outputfilename) {
  
namesoffiles <- list.files()
namesoffiles <- namesoffiles[grep("mapping", namesoffiles)]
...
ReadsTable <<- BarcodeFrequencies
write.csv(BarcodeFrequencies, outputfilename, row.names = FALSE)
}
...
plotSTAMP <- function(name) 
 {tiffname <- paste(name, ".tiff", sep="")
  ...
  plot(x, y, ylim = c(-8, 0), xlab = "Barcode", ylab = "Log10 Frequency", main =name)
  dev.off()}
...
H###merging all 5 pools together
