 ...
 #Makes a table for estimating bottleneck from fraction of barcodes that are identified
  ...
  ResiliencyIndices <- function(samplename, plots = FALSE){
    ...
    if (CorrectForNoise != 0) {
      ResampledInvec <- rmvhyper(20, round(invec), round(CorrectForNoise*sum(outvec)))
      ResampledInvec <- colMeans(ResampledInvec)
    ...
    q <- round(seq(1, length(na.omit(x)), length.out = max(length(na.omit(x))/15, 3)))
    p<-x[q]
    ...
    #Identifies greatest log change in x
    xdif <- c(log(x[1]), log(x))
    xdif <- xdif[1:length(xdif)-1]
    xsub <- na.omit((log(x) - xdif))
    greatestdif = which(xsub == max(xsub)) -1
    if(max(xsub) < 0.7) {greatestdif <- max(guessesuniquesorted)}
    ...
    #Greatest log change in original frequencies - only >10 fold is accepted
    outvecordered <- log(sort(outvec, decreasing = TRUE))
    outvecordered <- outvecordered[outvecordered > 0]
    staggered <- c(outvecordered[2:length(outvecordered)], 0)
    outvecdif <- na.omit(outvecordered - staggered)
    if(max(outvecdif) > log(10)) {guessesuniquesorted <- c(guessesuniquesorted, which(outvecdif ==max(outvecdif)))}
    ...
    guessesuniquesorted <- sort(unique(guessesuniquesorted))
    ...
    if(length(cfu)==0) {cfu <- 0}
    if(cfu == 1 ) {finalvalue <- 1}
    ...
    if(plots == FALSE) {
      ...
      invec <<- ReferenceVector
      c(noisestart, x[1], CompBottleneck, finalvalue, nbcalibrated, nrcalibrated)
    }
  }
  ...
  if(!is.null(CalibrationFile)) {colnames(TableOfEstimates) <- c("Nbarcodes","Nb", "Ns", "Nr",  "NbLower_Cal", "NbMedian_Cal", "NbUpper_Cal", "NrLower_Cal", "NrMedian_Cal", "NrUpper_Cal") }
  if(is.null(CalibrationFile)) {colnames(TableOfEstimates) <- c("Nbarcodes","Nb", "Ns", "Nr", "Nb_cal", "Nr_cal") }
  ...
  minweight <<- minweight
  dfxy <<- dfxy
  CorrectForNoise <<- CorrectForNoise
